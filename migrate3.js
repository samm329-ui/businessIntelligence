// migrate3.js - Direct PostgreSQL Connection
const { Client } = require('pg');

// Supabase connection string (using connection pooler)
// Format: postgresql://postgres:[password]@db.[project-ref].supabase.co:5432/postgres
// For service role, we need to use the direct connection

const connectionString = 'postgresql://postgres:postgres@db.bbpvgxlsnnvabesngbof.supabase.co:5432/postgres';

const sqlCommands = [
  {
    name: 'Enable Extensions',
    sql: `CREATE EXTENSION IF NOT EXISTS pg_trgm; CREATE EXTENSION IF NOT EXISTS "uuid-ossp";`
  },
  {
    name: 'Create parent_companies table',
    sql: `CREATE TABLE IF NOT EXISTS parent_companies (
  ...
  },
  {
    ...
    sql: `CREATE TABLE IF NOT EXISTS data_lineage (
      id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  ...
  },
  {
    ...
    sql: `CREATE TABLE IF NOT EXISTS validation_rules (
      id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  ...
  },
  {
    ...
    sql: `CREATE TABLE IF NOT EXISTS validation_results (
      id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
      entity_type VARCHAR(50) NOT NULL,
      entity_id UUID NOT NULL,
  ...
  },
  {
    ...
    sql: `CREATE TABLE IF NOT EXISTS cross_source_comparison (
      id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
      entity_type VARCHAR(50) NOT NULL,
      entity_id UUID NOT NULL,
      field_name VARCHAR(100) NOT NULL,
  ...
  },
  {
    ...
    sql: `CREATE TABLE IF NOT EXISTS ai_citations (
      id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  ...
  },
  {
    ...
    sql: `CREATE TABLE IF NOT EXISTS entity_resolution_log (
      id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  ...
  },
  {
    ...
    sql: `CREATE TABLE IF NOT EXISTS kpi_formulas (
      id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  ...
  },
  {
    ...
    sql: `INSERT INTO data_sources (name, source_type, base_url, rate_limit_per_min, reliability_score, cost_tier, documentation_url) VALUES
  ...
  },
  {
    ...
    sql: `INSERT INTO validation_rules (rule_name, rule_type, field_name, min_value, max_value, severity) VALUES
  ...
  },
  {
    ...
    sql: `INSERT INTO kpi_formulas (kpi_name, formula, version, description) VALUES
  ...
  {
    name: 'Create indexes',
    sql: `CREATE INDEX IF NOT EXISTS idx_parent_companies_name ON parent_companies(name);
  ...
  {
    name: 'Create views',
    sql: `CREATE OR REPLACE VIEW company_full_profile AS
      SELECT c.id, c.name as company_name, c.ticker, pc.name as parent_company,
        c.country, c.sector, c.industry, c.market_cap, c.revenue, c.employees, c.website,
        dl.confidence_score as data_confidence, ds.name as primary_data_source
      FROM companies c
      ...
      SELECT b.id as brand_id, b.name as brand_name, b.product_category, b.aliases,
        c.id as company_id, c.name as company_name, c.ticker, pc.name as parent_company,
        pc.ticker as parent_ticker, b.is_flagship
      FROM brands b
      JOIN companies c ON b.company_id = c.id
      LEFT JOIN parent_companies pc ON b.parent_company_id = pc.id;

      CREATE OR REPLACE VIEW data_quality_dashboard AS
      SELECT ds.name as source_name, COUNT(DISTINCT dl.entity_id) as entities_tracked,
      ...
      LEFT JOIN validation_results vr ON vr.rule_id IN (SELECT id FROM validation_rules WHERE is_active = true)
      GROUP BY ds.id, ds.name;

      CREATE OR REPLACE VIEW error_monitoring AS
      SELECT error_type, severity, component, COUNT(*) as error_count,
        COUNT(CASE WHEN resolved = false THEN 1 END) as unresolved_count, MAX(created_at) as last_occurrence
      FROM error_logs WHERE created_at > NOW() - INTERVAL '7 days'
      GROUP BY error_type, severity, component ORDER BY error_count DESC;

      CREATE OR REPLACE VIEW ai_hallucination_report AS
      SELECT aa.analysis_type, aa.entity_type, aa.model_name, COUNT(*) as total_analyses,
      ...
      FROM ai_analysis aa WHERE aa.created_at > NOW() - INTERVAL '30 days'
      GROUP BY aa.analysis_type, aa.entity_type, aa.model_name;`
  }
];

async function runMigration() {
  console.log('🚀 Starting Supabase Database Migration via Direct PostgreSQL...\n');
  ...
  try {
    ...
    for (let i = 0; i < sqlCommands.length; i++) {
      ...
      try {
        ...
        console.log(`  ✅ Success`);
        results.successful.push(cmd.name);
      } catch (err) {
        if (err.message.includes('already exists') || err.message.includes('duplicate')) {
          ...
          results.failed.push({ step: cmd.name, error: err.message });
        }
      }
    ...
    console.log(`✅ Successful: ${results.successful.length}/${sqlCommands.length}`);
    console.log(`❌ Failed: ${results.failed.length}/${sqlCommands.length}`);
    
    if (results.failed.length > 0) {
      console.log('\n❌ Failed Steps:');
      results.failed.forEach(({ step, error }) => {
        console.log(`  - ${step}: ${error.substring(0, 100)}`);
      });
    }
    ...
    const verifyQuery = `
      ...
      AND table_name IN ('parent_companies', 'brands', 'data_sources', 'data_lineage', 'error_logs')
      ORDER BY table_name;
    `;
    const verifyResult = await client.query(verifyQuery);
    console.log(`✅ Found ${verifyResult.rows.length}/5 key new tables`);
    
  } catch (err) {
    console.error('❌ Connection error:', err.message);
    console.log('\n💡 Troubleshooting:');
    console.log('  1. Check if database is publicly accessible');
