-- ============================================================
-- EBITA INTELLIGENCE - UPGRADE 2 DATABASE SCHEMA v4.0 (FIXED)
...
CREATE TABLE IF NOT EXISTS public.sector_hierarchy (
  ...
  created_at TIMESTAMPTZ DEFAULT now(),
  CONSTRAINT uq_sector_hierarchy UNIQUE (sector, industry, sub_industry, niche)
);
...
ON CONFLICT (sector, industry, sub_industry, niche) DO NOTHING;

-- 9. VIEWS
CREATE OR REPLACE VIEW public.v_fresh_consensus AS
SELECT * FROM public.consensus_metrics WHERE expires_at > now() ORDER BY fetched_at DESC;

CREATE OR REPLACE VIEW public.v_company_full AS
SELECT e.id, e.canonical_name, e.ticker_nse, e.ticker_bse, e.sector, e.industry,
  e.sub_industry, e.niche, e.region, e.country, e.is_listed, e.exchange, e.brands,
  cm.market_cap, cm.current_price, cm.revenue, cm.net_margin, cm.pe_ratio,
  cm.confidence_score, cm.fetched_at as data_as_of
FROM public.entity_intelligence e
LEFT JOIN public.consensus_metrics cm ON cm.entity_id = e.id
  AND cm.fiscal_period = 'TTM' AND cm.expires_at > now()
WHERE e.is_active = true;

SELECT 'Upgrade 2 complete!' as status;
