...
const SUPABASE_URL = 'bbpvgxlsnnvabesngbof.supabase.co';
const SUPABASE_KEY = 'eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6ImJicHZneGxzbm52YWJlc25nYm9mIiwicm9sZSI6InNlcnZpY2Vfcm9sZSIsImlhdCI6MTc3MDQ4MDUzOCwiZXhwIjoyMDg2MDU2NTM4fQ.azY7-CkeQ1OAwjtBgY8KHmMI8EOPLdtZl5ly5yECjDE';
...
async function runMigration() {
  ...
  for (let i = 0; i < sqlCommands.length; i++) {
    ...
    try {
      const result = await executeSQL(cmd.sql);
      if (result.success) {
        console.log(`  ✅ Success`);
        ...
        if (errorMsg.includes('already exists') || errorMsg.includes('duplicate')) {
          ...
          results.failed.push({ step: cmd.name, error: errorMsg });
        }
      }
    } catch (err) {
